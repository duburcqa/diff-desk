...
Reply when the answer needs discussing, resolve when it is settled - a resolved thread keeps its remark and every reply, and the reviewer can reopen it. A note is neither: printed as `[i] note <who> ...` in a thread, it is written for this side of the desk - what the reviewer wants looked at, what is left to do - and it never reaches the pull request, so it is guidance to act on rather than a remark to answer. It hangs on what it is about, said as `on [j]` when that is not the remark itself, and it is answered with a note of its own: a reviewer's "check whether this is true, and if it is not just resolve it" is answered with `reply 3 --note --on 0 "done."` and the thread closed with `resolve 3`, which leaves nothing of it on the pull request. Nothing is said to a note itself: it stands on the remark or on a reply, and what there is to add is another note on the same thing. Resolving a comment that was posted to a pull request also resolves its thread there, and says "not resolved there yet" until GitHub confirms it.
