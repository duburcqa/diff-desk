...
Run it as a background job of the session, the way any other long-running command is run, and never detached with `nohup`, `disown` or `setsid`. A desk inside the session's process tree goes down when the session ends, as every command it ran does; one put outside that tree outlives it, and the next session finds a page serving a review nobody is reading, on code that has since been superseded.
...
A page load collects the diffs again, so a reload always shows the branch as it now stands, uncommitted work included. The diffs are read from git alone; what GitHub adds to them (pull request links, the reader's login) is read from what it last answered and asked again in the background, so neither a page load nor a refresh ever waits on GitHub, with the network gone or not. An answer landing later reaches the open page on its own, with the diffs left as they are. While the page is open it watches what it was built from and offers a **Refresh** in the bar once that has moved on - a commit, a fixup, work saved on disk - rather than rebuilding the diff under the reader. Taking it keeps the branch and commit being read. So an edit made while the user is reviewing needs no command from the session; tell them to refresh.
...
A thread is printed whole and the line that woke the session is marked with a `*`, so what has already been answered reads as read. A watch armed after the desk restarted says so once and then follows the desk now serving, so a restart costs no arming cycle and nothing said meanwhile is missed. It keeps running and prints whatever the reviewer says, as they say it - not only the first thing, and not only new comments: a reply on a comment already read reaches it just the same, since it follows the log's event cursor rather than the comment numbers. A session's own replies and resolutions bump that cursor too and are told apart, so what it is waiting for is never confused with what it just did.

Armed without `--once` it never stops of its own accord, which is what a session wants: every batch reaches it as it lands, with nothing to arm again and no moment at which forgetting to is silent. `--once` stops it after the first report and remembers where it stopped, for a session that would rather be woken once and decide what to do next: the same reply never wakes one twice, and nothing said while it was answering is lost. `--since N` overrides where it resumes from.
...
- A comment carries files as well as words - an image or a video pasted, dropped or picked into any box the reviewer writes in: `.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`, `.svg`, `.mp4`, `.mov`, `.webm`, which is what a token can upload and therefore all a box takes. A reviewer reaching for a PDF, a spreadsheet, a markdown file or a dump is told so as they offer it; those reach a pull request only through a browser, so nothing here holds one. They are kept beside the log, printed as paths by `desk.py comments`, and reach the pull request only when the reader sends that thread, uploaded then and linked under the words. A comment kept local keeps its files on the desk, as its text stays there.
